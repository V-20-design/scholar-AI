    ...
    if uploaded_file and not st.session_state.summary:
        if st.button("✨ Auto-Analyze & FAQs"):
            with st.spinner("Analyzing..."):
                try:
                    model = genai.GenerativeModel(init_scholar())
                    ...
                    st.error("Limit hit. Wait 60s.")

    st.divider()
    st.header("⏱️ Focus Timer")
    t1, t2 = st.columns(2)
    if t1.button("▶️ 25m"): st.toast("Timer active.")
...
if not st.session_state.history:
    ...
    cols = st.columns(3)
    if cols[0].button("🧬 Quantum"): st.session_state.active_prompt = "Explain Quantum Biology."
...
with tab_insights:
    ...
    else: st.write("Upload a file to see insights.")
...
with tab_chat:
    ...
    if query:
        ...
        with st.chat_message("assistant"):
            ...
            try:
                model = genai.GenerativeModel(init_scholar())
                # CRITICAL: We only send query + file. NO history. 
                # This keeps token usage at the bare minimum.
                payload = [query]
                if uploaded_file:
                ...
                if "429" in str(e):
                    st.error("🚨 Rate Limit! Wait 60s for the token bucket to refill.")
                    pb = st.progress(0)
                    for i in range(60):
                        time.sleep(1)
                        pb.progress((i+1)/60)
